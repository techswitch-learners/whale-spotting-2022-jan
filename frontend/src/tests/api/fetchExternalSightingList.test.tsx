...
const callWithOkResponse = async () => {
  ...
  return await fetchExternalSightingList();
};

describe("fetchExternalSightingList: ", () => {
  test("makes exactly one fetch request", async () => {
    await callWithOkResponse();
    expect(global.fetch).toHaveBeenCalledTimes(1);
  });

  test("sends the GET request to the Whale Spotting external API", async () => {
    await callWithOkResponse();
